...
const u8 MSK_TAB[9] = {0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0};
...
/**
  ...
  */
u8 uart_send(u8 b)
{ //transmition/bufferring possible?
...
void uart_Tx_timing(void)
{
	...
	{
		if (test_status(transmit_in_progress))
		{ // edge of current bit (no service for middle)
			switch (Tx_bit)
			{ // begin of bit transmition
			case 0:
...
#endif
			case DATA_LENGTH + 1:
				set_status(transmit_data_reg_empty);
			case DATA_LENGTH + 2:
			...
			{ // end of current transmited bit
				if (test_status(transmit_data_reg_empty))
				{									  // end of transmition
...
void uart_Rx_timing(void)
{
	...
	{
		if (!Rx_phase)
		{				 // receive is in progres now //Edge status is 1, if middle bit - skip (prescaler of frequency)
			Rx_samp = 0; // middle of current bit, checking samples
#ifdef TEST_PIN_USED
			set_Test_Pin;
#endif
			if (Rx_test)
				++Rx_samp; // sampling in the middle of current bit
			if (Rx_test)
				++Rx_samp;
			if (Rx_test)
				++Rx_samp;
			...
			if (Rx_bit == 0)
			{
				...
				{ // noise in start bit, find next one
					disable_OC_system;
					enable_IC_system;
...
#ifdef BIT9
							if (Rx_bit9)
								set_status(receive_9th_data_bit);
							else
								clr_status(receive_9th_data_bit);
#endif
