...
#define clear_owerflow_flag     \
  {                             \
    TIM2->SR1 &= ~TIM2_SR1_UIF; \
  }
#define clear_cc_flag             \
  {                               \
    TIM2->SR1 &= ~TIM2_SR1_CC2IF; \
  }
#define enable_cc_interrupt      \
  {                              \
    TIM2->IER |= TIM2_IER_CC2IE; \
  }
#define disable_cc_interrupt      \
  {                               \
    TIM2->IER &= ~TIM2_IER_CC2IE; \
  }

#define enable_IC_system \
  { /* clear_cc_flag;*/  \
    init_ic_setting;     \
    enable_cc_interrupt; \
  }
#define disable_IC_system                  \
  {                                        \
    ...
    TIM2->CCMR2 &= ~TIM2_CCMR_CCxS;        \
  };

#define enable_OC_system \
  { /* clear_cc_flag;*/  \
    init_oc_setting;     \
    enable_cc_interrupt; \
  }
...
#ifdef SWUART_TRANSMIT_USED
#define init_UART_Tx_pin                                            \
  {                                                                 \
    GPIO_Init(UART_TxPORT, UART_TxPIN, GPIO_MODE_OUT_PP_HIGH_SLOW); \
  }
#endif
#ifdef SWUART_RECEIVE_USED
#define init_UART_Rx_pin                                       \
  {                                                            \
    GPIO_Init(UART_RxPORT, UART_RxPIN, GPIO_MODE_IN_FL_NO_IT); \
  }
#endif
#ifdef TEST_PIN_USED
#define init_UART_Test_pin                                             \
  {                                                                    \
    GPIO_Init(UART_TestPORT, UART_TestPIN, GPIO_MODE_OUT_PP_LOW_FAST); \
  }
#endif
...
#define uart_receive_enable    \
  {                            \
    Rx_bit = Rx_phase = false; \
    disable_OC_system;         \
    clear_cc_flag;             \
    enable_IC_system;          \
  }
#define uart_receive_disable         \
  {                                  \
    disable_IC_system;               \
    disable_OC_system;               \
